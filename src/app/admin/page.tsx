...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    return onSnapshot(doc(db, "analytics", "overview"), (snapshot) => {
      if (snapshot.exists()) {
  ...
  useEffect(() => {
    if (!isAuthenticated) return;
    return onSnapshot(query(collection(db, "temporarySubCategories")), (snapshot) => {
      const data = snapshot.docs.map((item) => ({ id: item.id, ...item.data() } as TempSubCategory));
      data.sort((a, b) => ((a.order ?? 0) !== (b.order ?? 0) ? (a.order ?? 0) - (b.order ?? 0) : (a.createdAt?.toMillis?.() ?? 0) - (b.createdAt?.toMillis?.() ?? 0)));
  ...
  useEffect(() => {
    ...
    return onSnapshot(doc(db, "settings", "eventTheme"), (snapshot) => {
      if (!snapshot.exists()) {
        setEventTheme("none"); setEventEnabled(false); setEventAnimation(true); setEventParticles(false); return;
      }
      const data = snapshot.data();
  ...
  const getSubCategoriesForTab = (tab: string) => {
    ...
    const temporary = tempSubCategories.filter((item) => item.type === tab && item.enabled).map((item) => item.id);
  ...
  const saveSubCategoryOrder = async (tab: string, order: string[]) => {
    try {
      ...
      await setDoc(doc(db, "settings", "subCategoryOrder"), { orders: nextOrders }, { merge: true });
    } catch (error) {
  ...
  const duplicateProduct = async () => {
    ...
    if (copyTargetSub === copyingProduct.subCategory) return alert("The package is already in this subcategory.");
    ...
    try {
      ...
      const snapshot = await getDocs(query(collection(db, "products"), where("type", "==", targetTabType)));
      const duplicateExists = snapshot.docs.some((item) => {
        const data = item.data();
        return (data.subCategory === copyTargetSub && Number(data.price) === Number(copyingProduct.price) && JSON.stringify(data.description || {}) === JSON.stringify(copyingProduct.description || {}) && JSON.stringify(data.images || []) === JSON.stringify(copyingProduct.images || []));
      });

      if (duplicateExists) return alert("A similar package already exists in the destination subcategory.");

      const { id, ...productWithoutId } = copyingProduct;
      await addDoc(collection(db, "products"), { ...productWithoutId, type: targetTabType, subCategory: copyTargetSub, createdAt: serverTimestamp() });

      setCopyingProduct(null); setCopyTargetSub("");
      setActiveTab(targetTabType); setActiveSub(copyTargetSub);
      fetchProducts();
    } catch (error) {
  ...
  const saveEventTheme = async () => {
    ...
    try {
      await setDoc(doc(db, "settings", "eventTheme"), { event: eventTheme, enabled: eventEnabled, animation: eventAnimation, particles: eventParticles });
      if (typeof window !== "undefined") {
        localStorage.setItem(`${siteConfig.storagePrefix}_theme_name`, eventTheme);
        localStorage.setItem(`${siteConfig.storagePrefix}_theme_enabled`, eventEnabled ? "true" : "false");
      }
    } catch (error) {
  ...
  const resetTempForm = () => {
    ...
    setTempForm({ type: activeTab, enabled: true, name: { am: "", en: "", om: "" } });
  };

  const openTempConfigForEdit = (temp: TempSubCategory) => {
    setEditingTempId(temp.id);
    setTempForm({ type: temp.type, enabled: temp.enabled, name: { am: temp.name?.am || "", en: temp.name?.en || "", om: temp.name?.om || "" } });
  };

  const saveTempSubCategory = async () => {
    if (!tempForm.name.am.trim() && !tempForm.name.en.trim() && !tempForm.name.om.trim()) return alert("Please enter at least one name.");
    try {
      if (editingTempId) {
        await updateDoc(doc(db, "temporarySubCategories", editingTempId), { type: tempForm.type, enabled: tempForm.enabled, name: tempForm.name });
      } else {
        const sameType = tempSubCategories.filter((item) => item.type === tempForm.type);
        const nextOrder = sameType.length > 0 ? Math.max(...sameType.map((item) => item.order ?? 0)) + 1 : 1;
        await addDoc(collection(db, "temporarySubCategories"), { type: tempForm.type, enabled: tempForm.enabled, order: nextOrder, name: tempForm.name, createdAt: serverTimestamp() });
      }
      resetTempForm();
    } catch (error) {
  ...
  const toggleTempSubCategory = async (temp: TempSubCategory) => {
    try { await updateDoc(doc(db, "temporarySubCategories", temp.id), { enabled: !temp.enabled }); } 
    catch (error) { alert("Failed to change visibility."); }
  };

  const deleteTempSubCategory = async (id: string) => {
    if (!confirm("Delete this temporary subcategory? Products assigned to it will remain in Firebase but will no longer appear under a menu category.")) return;
    try { await deleteDoc(doc(db, "temporarySubCategories", id)); } 
    catch (error) { alert("Failed to delete temporary subcategory."); }
  };
  ...
  useEffect(() => {
    ...
    if (isTemporary && !tempSubCategories.find((item) => item.id === activeSub)?.enabled) setActiveSub("all");
  ...
  const fetchProducts = async () => {
    ...
    let data = snapshot.docs.map((docSnap) => ({ id: docSnap.id, ...docSnap.data(), images: docSnap.data().images || (docSnap.data().imageUrl ? [docSnap.data().imageUrl] : []) } as Product));
  ...
  const handleDelete = async (id: string) => {
    if (confirm("Are you sure you want to delete this product?")) {
      ...
      catch (error) { alert("Failed to delete product. Please check your connection."); }
    }
  };
      ...
      <header className="bg-[var(--surface-card)] border-b border-[var(--border-subtle)] shadow-sm sticky top-0 z-40 transition-colors">
        <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-10 h-16 md:h-20 flex items-center justify-between">
          <div className="flex flex-col">
            ...
            <span className="text-[10px] md:text-xs text-[var(--text-muted)] font-medium hidden sm:block">{siteConfig.name.en} Management</span>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {!isAdding && (
              <button onClick={() => { setProduct(getInitialFormState()); setIsAdding(true); }} className="px-4 py-2 bg-[var(--brand-gold)] text-[var(--text-on-gold)] text-xs md:text-sm font-bold rounded-full shadow-md hover:opacity-95 transition-all">
                + Add New
              </button>
            )}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select value={eventTheme} onChange={(e) => setEventTheme(e.target.value)} className="w-full p-3 border border-[var(--border-subtle)] rounded-xl text-sm bg-[var(--brand-bg)] text-[var(--text-primary)] outline-none">
                  ...
                  <option value="muslim">☪️ Muslim Holy Day</option>
              ...
              <button onClick={saveEventTheme} disabled={savingEventTheme} className="w-full py-3 bg-[var(--brand-gold)] text-[var(--text-on-gold)] rounded-xl font-bold text-sm shadow-md hover:opacity-95 transition-all">
                {savingEventTheme ? "Saving..." : "Save Event Theme"}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[var(--surface-card)] border border-[var(--border-subtle)] w-full max-w-2xl rounded-3xl p-6 md:p-8 shadow-2xl max-h-[90vh] overflow-y-auto transition-colors">
            ...
            <div className="space-y-3">
              <h3 className="font-extrabold text-sm text-[var(--text-muted)]">Existing Temporary Categories</h3>
              {tempSubCategories.length === 0 ? (
                <div className="p-5 text-center rounded-2xl bg-[var(--surface-secondary)] border border-[var(--border-subtle)] text-xs text-[var(--text-muted)]">No temporary subcategories created yet.</div>
              ) : (
                tempSubCategories.map((temp, index) => (
                  <div key={`${temp.id}-${index}`} className="p-4 rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-secondary)] flex flex-col md:flex-row md:items-center gap-3 justify-between">
        ...
        {!isAdding && (
          <div className="grid grid-cols-3 gap-3 md:gap-6 mb-8">
            ...
            <div className="bg-[var(--surface-card)] border border-[var(--border-subtle)] rounded-2xl md:rounded-3xl p-3.5 md:p-5 shadow-sm flex flex-col justify-between transition-colors">
              ...
              <span className="text-xl md:text-3xl font-black text-emerald-500">{conversionRate}%</span>
            </div>
          </div>
        )}
        ...
        {isAdding ? (
              ...
              <div className="flex gap-2 mb-2 flex-wrap">
                {product.images.map((url, i) => (
                  <div key={`${url}-${i}`} className="relative w-16 h-16 md:w-20 md:h-20">
                    <img src={url} className="w-full h-full object-cover rounded-xl shadow-sm border border-[var(--border-subtle)]" />
                    <button type="button" onClick={() => removeImage(i)} className="absolute -top-2 -right-2 bg-red-600 text-white rounded-full w-5 h-5 text-xs flex items-center justify-center shadow hover:bg-red-500">✕</button>
                  </div>
                ))}
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
            {products.map((p, index) => {
              const tabConfig = siteConfig.tabs.find(t => t.id === p.type);
              const titlePrefix = tabConfig ? tabConfig.prefix : "PKG";

              return (
                <div key={`${p.id}-${index}`} className={`theme-card bg-[var(--surface-card)] rounded-3xl p-3 md:p-4 shadow-sm border border-[var(--border-subtle)] flex flex-row items-stretch gap-4 transition-all ${p.visible === false ? "opacity-50" : ""} ${eventAnimation ? "theme-animate" : ""}`}>
                  <div className="w-[50%] aspect-[1/1] overflow-hidden rounded-2xl flex-shrink-0 relative">
                    ...
                    <div className="absolute top-2 left-2 bg-black/70 backdrop-blur-sm px-2 py-0.5 rounded-md border border-white/10">
                      <span className="text-[10px] text-white font-bold uppercase">{tempSubCategories.find((item) => item.id === p.subCategory)?.name?.en || (tAdmin.subs as any)[activeTab]?.[p.subCategory] || p.subCategory}</span>
                    </div>
                  </div>
      ...
      <footer className="bg-[var(--surface-card)] text-[var(--text-secondary)] mt-12 py-8 px-4 md:px-6 lg:px-10 border-t border-[var(--border-subtle)] transition-colors">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4 text-center md:text-left">
          <div>
            ...
            <p className="text-xs text-[var(--text-muted)]">Secure catalog and order tracking system.</p>
          </div>
